...
<p>The explore page contains all proteins in the PDB up to 10,000 residues. <strong>(1)</strong> Search and filtering bar, which works for all the columns in the table. <strong>(2)</strong> PDB ID column. <strong>(3)</strong> Description column, containing the protein name derived from the PDB. <strong>(4)</strong> Protein size, number of contacts, and pH. For the pH column, the value shown was obtained directly from the PDB, when available, defaulting to 7.4 when not defined in the deposition. <strong>(5)</strong> Number of proteins in the current filter and pagination of the results.</p>
<p align="center">
  <img src="<?=base_url('/img')?>/docs/explore.png" class="w-50">
</p>

<h4 class="pt-4 pb-1" id="results_page">Results Page</h4>
<p><strong>(1)</strong> Download options for the results, which include the contact list in CSV format, the submitted PDB/mmCIF file, and the Pymol session with annotated contacts in PSE format. <strong>(2)</strong> Button to show the protein contact map in a pop-up window. <strong>(3)</strong> Protein description, obtained from the PDB. <strong>(4)</strong> Full information for the protein, including the number of residues, number of contacts of each individual type, and pH used for calculation.<strong>(5)</strong> Total number of contacts identified for the protein. <strong>(6)</strong> Options for filtering the results. <strong>(7)</strong> Full list of contacts, with the columns being: contact name; protein chain of the first atom; residue of the first atom; name of the first atom; protein chain of the second atom; residue of the second atom; name of the second atom; distance between the atom pair, in angstroms; localization of the contact (intra-chain or inter-chain); contact type; show the individual contact in the visualization.<strong>(8)</strong> Interactive visualization of the protein. <strong>(9)</strong> Number of contacts in the current filter and pagination of the results.</p>
...
<p>Next to the download button on the results page, the user can view the protein contact map in an interactive pop-up window. Since this is a two-dimensional representation of a protein's contacts, each axis of the map represents a polypeptide chain, both of which can be dynamically adjusted (<strong>1</strong> for the X-axis and <strong>2</strong> for the Y-axis). In this way, inter-chain contacts can be visualized by selecting different chains in the menus, and the maps can also be saved in ".png" format <strong>(3)</strong>. <strong>(4)</strong> Interactive view of the selected pair of chains, with each point being a pair of residues, and the colors representing the predominant type of contact for the pair. <strong>(5)</strong> By hovering the mouse over any of the points, the user can see the detailed information for all the contacts made by the residue pair.</p>
<p><strong>(6)</strong> Color scheme for the contact types. HY = hydrophobic; HB = hydrogen bond; AT = attractive; RE = repulsive; SB = salt bridge; AS = aromatic stacking; uAT = uncertain attractive; uRE = uncertain repulsive; uSB = uncertain salt bridge.</p>
<p align="center">
